...
import { BaseProvider, DOProvider } from '../providers/index.js'
...
describe('getProvider', () => {
  ...
  it('returns Space provider when options have space', () => {
    const getProviderResponse = getProvider({ space: { bucket, region } })

    expect(getProviderResponse.name).to.equals('space')
    expect(getProviderResponse.provider).to.be.instanceOf(DOProvider)
  })
